let settings = {
  ...
  Shelly.addEventHandler(function(event) {
    if (typeof event === undefined || typeof event.info === undefined || (event.info.event !== "single_push" && event.info.event !== "double_push" && event.info.event !== "longh_push")) {
      return;
    }
    ...
    if (componentId = BRIGHTNESS_COMPONENT_ID) {
      ...
      if (settings.brightness < 0) settings.brithgtness = 0;
    } else {
